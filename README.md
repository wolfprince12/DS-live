...
**网页模式怎么用**：设置 →「🌐 网页模式」打开内嵌的官方页面，正常登录（登录态长期保留）。想让 AI 记得你时，在输入框打完字后点右下角 **🧠 注入记忆**（或按 `Cmd/Ctrl + M`），程序会把最匹配的几条本地记忆拼到消息开头 —— **发送键仍然由你自己按**，程序不会替你发消息。
...
│  ├─ src/webmode.rs    # 网页模式：外部 WebView + 记忆注入脚本
│  ├─ tauri.conf.json
