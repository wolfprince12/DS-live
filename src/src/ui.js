...
async function send() {
    ...
    try {
        await api.chat(store.currentId, text, store.model, store.useMemory, store.thinking, (msg) => {
...
function startEditMemory(m) {
    const el = memoryListEl.querySelector(`[data-id="${m.id}"]`);
    if (!el)
        return;
    el.innerHTML = "";
...
async function saveEditMemory(id, content) {
